...
※自前のRaspberryPiで実行する場合はPythonファイルを作成してコンソールから実行してください。
...
また、図のようにGPIOにつなげていない方のボタンの足をGNDに接続した場合、おした時はGPIOにGNDが入力されますが、押していないときはどこにも接続されておらず、入力が不安定になります。
...
ここではLEDをGPIO02, ボタンをGPIO05に接続した例の回路を示します。
